...
export class CapturaActividadService {
  ...
  getActividadesSemana(id_col?:string, anio?:string, semana?:string){
    ...
    console.log('debug getActividadesSemana', id_col);
    
    // const url = `${ this.baseUrl }/actividades/semana/${id_col}/${anio}/${semana}`;
    const url = `${ this.baseUrl }/registros/semana/${id_col}/${anio}/${semana}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
  ...
  getDetalleActividad(idActividad:string){
    const url = `${ this.baseUrl }/actividades/${idActividad}`;
    //console.log("token:" +localStorage.getItem('token'));
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders({
  ...
  detalleActividad( act: registroActividad ){
    ...
    this._idActividad = this._actividad.actividad
